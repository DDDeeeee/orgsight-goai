...
> 状态：`talent-role-insight` 的 Leader、人物职业画像、岗位分析与团队角色生态 Worker 已在本地 AgentTeams 环境上传、apply 并启动；三条 Worker 链路均已完成真实端到端验收。其他 Agent 的 package 状态以各自目录为准。

本表只记录将来的生成去向，避免把角色定义、AgentTeams package 和 AgentTeams 资源 YAML 混为一层。

| Agent | 类型 | 角色源 | package 源目录 | 资源 YAML | Team / 关系 | GOAI Skill 副本 |
| --- | --- | --- | --- | --- | --- | --- |
...
| `talent-role-insight-lead` | Team Leader / Worker | `talent-role-insight-lead/AGENT.md` | `packages/talent-role-insight-lead/`（本地已部署） | `manifests/workers/talent-role-insight-lead.yaml` | `talent-role-insight` 的 Leader | 无 |
| `person-profile-worker` | Worker | `person-profile-worker/AGENT.md` | `packages/person-profile-worker/`（本地已部署；真实 E2E 已验收） | `manifests/workers/person-profile-worker.yaml` | `talent-role-insight` | `modeling-roles` |
| `role-and-position-analysis-worker` | Worker | `role-and-position-analysis-worker/AGENT.md` | `packages/role-and-position-analysis-worker/`（本地已部署、已验收） | `manifests/workers/role-and-position-analysis-worker.yaml` | `talent-role-insight` | `analyzing-role-and-position` |
| `team-role-ecology-worker` | Worker | `team-role-ecology-worker/AGENT.md` | `packages/team-role-ecology-worker/`（本地已部署；真实 E2E 已验收） | `manifests/workers/team-role-ecology-worker.yaml` | `talent-role-insight` | `assessing-team-role-ecology` |
...
- 除 `talent-role-insight` 当前联调成员外，每个 package 源目录与 YAML 仅在表中路径生成。`talent-role-insight` 的本地部署使用其 Worker YAML 中已记录的 Controller package URI；未来替换 ZIP 后，必须先上传新 artifact 并重新 apply 对应 Worker，不能把本地 ZIP 误认为已部署版本。
...
- 尚未部署的 YAML 保持 `state: Stopped`，不执行 `agt apply`；`talent-role-insight` 的联调成员是已批准的例外，当前为 `Running`。
